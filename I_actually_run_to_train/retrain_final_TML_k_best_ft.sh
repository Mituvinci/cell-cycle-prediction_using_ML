#!/usr/bin/env bash
# FINAL TRAINING - ALL TRADITIONAL ML MODELS WITH SELECTKBEST (K=300)
...
for i in "${!DATASETS[@]}"; do
  ...
  for MODEL in "${MODELS[@]}"; do
    ...
    sbatch --export=ALL,TRAINING_SCRIPT="../2_model_training/train_traditional_ml.py",TRAINING_ARGS="$TRAINING_ARGS" --job-name="TML_K300_${DATASET}_${MODEL}" ../scripts/train_model_generic.slurm
    sleep 1
  done
done

echo "================================================================================"
echo "SUBMITTED: 3 datasets × 4 TML models = 12 jobs (with SelectKBest k=300)"
echo "================================================================================"
