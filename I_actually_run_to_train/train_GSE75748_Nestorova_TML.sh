#!/usr/bin/env bash
# Train TML models on GSE75748 + Nestorova
...
echo "Training TML models on GSE75748 + Nestorova (trials=$TRIALS, cv=$CV)"
...
for MODEL in "${MODELS[@]}"; do
  ...
  sbatch --export=ALL,TRAINING_SCRIPT="../2_model_training/train_traditional_ml.py",TRAINING_ARGS="$TRAINING_ARGS" --job-name="TML_GSE75748Nest_${MODEL}" ../scripts/train_model_generic.slurm
  sleep 1
done

echo "SUBMITTED: 4 TML jobs"
