...
def scaling_benchmark_triple(benchmark_data, scaler):
    """
    TRIPLE SCALING - Applies scaling to benchmark data using a fitted scaler,
    followed by TWO additional normalization steps.

    EXPERIMENTAL METHOD for extreme domain adaptation.

    This function performs THREE steps:
    1. Apply scaler.transform()
    2. Apply additional normalization: (scaled - mean) / std
    3. Apply ANOTHER normalization: (scaled2 - mean) / std

    Purpose: Test if additional distribution matching further improves
    cross-platform performance by addressing sparsity differences
    (71-77% zeros in training vs 35-50% in benchmarks).
    ...
    Returns:
        pd.DataFrame: Triple-scaled benchmark data
    """
    # Ensure benchmark data has the same feature order as scaler expects
    expected_feature_order = scaler.feature_names_in_
    benchmark_data = benchmark_data[expected_feature_order]

    # STEP 1: Apply trained scaler
    scaled_benchmark_data = scaler.transform(benchmark_data)
    scaled_benchmark_data = pd.DataFrame(scaled_benchmark_data, columns=benchmark_data.columns)

    # STEP 2: Apply first additional normalization
    X_benchmark_double = (scaled_benchmark_data - scaled_benchmark_data.mean()) / scaled_benchmark_data.std()
    X_benchmark_double = pd.DataFrame(X_benchmark_double, columns=benchmark_data.columns)

    # STEP 3: Apply SECOND additional normalization (triple scaling)
    X_benchmark_triple = (X_benchmark_double - X_benchmark_double.mean()) / X_benchmark_double.std()
    X_benchmark_triple = pd.DataFrame(X_benchmark_triple, columns=benchmark_data.columns)
